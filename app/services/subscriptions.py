...
class SubscriptionService:
    ...
    async def mark_active(self, order_id: int, provider_service_id: str | None = None, subscription_url: str | None = None, starts_at: datetime | None = None, expires_at: datetime | None = None):
        item = await self.ensure_for_paid_order(order_id)
        async with AsyncSessionLocal() as db:
            item = await db.merge(item); item.status = "active"; item.provider_service_id = provider_service_id; item.subscription_url = subscription_url; item.starts_at = starts_at or datetime.now(timezone.utc); item.expires_at = expires_at; await db.commit(); await db.refresh(item); return item
    ...
    async def revoke(self, subscription_id: int, actor_id: int | None = None):
        ...
        async with AsyncSessionLocal() as db:
            ...
            item.status = "revoked"; await db.commit(); await db.refresh(item)
        await LOG_SERVICE.add("subscription.revoked", f"subscription=#{subscription_id}", actor_id); return item

    async def mark_expired(self, subscription_id: int | None = None):
        tenant = require_tenant(); now = datetime.now(timezone.utc)
        async with AsyncSessionLocal() as db:
            if subscription_id is not None:
                item = await db.scalar(select(ServiceSubscription).where(ServiceSubscription.tenant_id == tenant, ServiceSubscription.id == subscription_id))
                if item is None: raise LookupError("سرویس پیدا نشد.")
                if item.status != "active" or not item.expires_at or item.expires_at > now: raise ValueError("این سرویس هنوز منقضی نشده است.")
                rows = [item]
            else:
            ...
            if rows:
                await db.commit()
                for item in rows: await db.refresh(item)
