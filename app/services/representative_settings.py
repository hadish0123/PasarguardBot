...
class RepresentativeSettingsService:
    ...
    async def set(self, key: str, value: str) -> dict[str, str]:
        ...
        value = value.strip()
        if not value: raise ValueError("مقدار نمی‌تواند خالی باشد.")
        if key == "brand" and len(value) > 120: raise ValueError("نام برند بیش از حد طولانی است.")
        if key == "support_username": value = value.lstrip("@").strip()
        if key == "timezone" and len(value) > 80: raise ValueError("منطقه زمانی نامعتبر است.")
        tenant = require_tenant()
        if SessionFactory is None: raise RuntimeError("DATABASE_URL is not configured")
        async with SessionFactory() as session:
