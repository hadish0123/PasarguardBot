...
class OrderService:
 ...
 async def checkout(self,telegram_user_id,plan_id,discount_code=None):
  ...
  async with SessionFactory() as session:
   ...
   if discount_code:
    code=discount_code.strip().upper();discount=await session.scalar(select(Discount).where(Discount.tenant_id==tenant_id,Discount.code==code))
  ...
  await LOG_SERVICE.add("order.checkout",f"order=#{order.id} subtotal={subtotal} discount={discount_amount} total={total} code={code or '-'}",telegram_user_id);return order
