...
class OrderService:
    async def list(self, status=None, limit=30):
        ...
        async with SessionFactory() as session:
            stmt = (
                select(Order)
                .where(Order.tenant_id == require_tenant())
    ...
    async def checkout(self, telegram_user_id, plan_id, discount_code=None):
        ...
        async with SessionFactory() as session:
            ...
            if discount:
                # Reserve the redemption while the order is open. It is released
                # again if the order is later cancelled before payment.
        ...
        await LOG_SERVICE.add(
            "order.checkout",
            f"order=#{order.id} subtotal={subtotal} discount={discount_amount} total={total} code={code or '-'}",
            telegram_user_id,
        )
        return order
        ...
        async with SessionFactory() as session:
            return await session.scalar(
                select(CheckoutRecord).where(
                    CheckoutRecord.tenant_id == require_tenant(),
                    CheckoutRecord.order_id == order_id,
                )
            )

    async def submit_payment(self, order_id, reference):
        reference = reference.strip()
        if not reference or len(reference) > 190:
            raise ValueError("شناسه پرداخت نامعتبر است.")
        if SessionFactory is None:
            raise RuntimeError("DATABASE_URL is not configured")
        async with SessionFactory() as session:
            ...
            record = await session.scalar(
                select(CheckoutRecord).where(
                    CheckoutRecord.tenant_id == order.tenant_id,
                    CheckoutRecord.order_id == order.id,
                )
            )
            if record is None:
                raise LookupError("جزئیات پرداخت سفارش پیدا نشد.")
            record.payment_reference = reference
            record.payment_submitted_at = datetime.now(timezone.utc)
            await session.commit()
            await session.refresh(order)
        await LOG_SERVICE.add("order.payment_submitted", f"order=#{order_id}", order.telegram_user_id)
        return order
    ...
    async def set_status(self, order_id, status):
        ...
        async with SessionFactory() as session:
            ...
            record = await session.scalar(
                select(CheckoutRecord).where(
                    CheckoutRecord.tenant_id == tenant_id,
                    CheckoutRecord.order_id == order.id,
                )
            )
            ...
            if status not in allowed.get(old, set()):
                raise ValueError(f"انتقال وضعیت {old} به {status} مجاز نیست.")

            if status == "paid" and order.amount > 0 and (
                record is None or not record.payment_reference
            ):
                raise ValueError("برای تأیید پرداخت، ابتدا شناسه پرداخت کاربر باید ثبت شده باشد.")
            ...
            if status == "fulfilled":
                ...
                if subscription is None or subscription.status != "active":
                    raise ValueError("سفارش تا فعال شدن سرویس پاسارگارد قابل تکمیل نیست.")
            ...
            if status == "cancelled":
                ...
                if subscription and subscription.status == "active":
                    raise ValueError("سرویس فعال است؛ ابتدا سرویس را لغو/ابطال کنید.")
                revoke_after = subscription is not None
                if redemption is not None and redemption.amount is not None:
                    discount = await session.get(Discount, redemption.discount_id)
            ...
            await session.commit()
            await session.refresh(order)
            provision_after = status == "paid"

        if revoke_after:
            await LOG_SERVICE.add("order.cancelled", f"order=#{order_id} reservation released")
        ...
        if provision_after:
            try:
                ...
                await LOG_SERVICE.add(
                    "order.provisioned",
                    f"order=#{order.id} provider_id={subscription.provider_service_id or '-'}",
                )
            except ValueError as exc:
                await LOG_SERVICE.add(
                    "order.provisioning_pending",
                    f"order=#{order.id} reason={exc}",
                )
            except Exception as exc:
                await LOG_SERVICE.add(
                    "order.provisioning_failed",
                    f"order=#{order.id} error={exc}",
                )

        await LOG_SERVICE.add("order.status_changed", f"order=#{order_id} {old}->{status}")
        return order
