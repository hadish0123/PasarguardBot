...
class OrderService:
    ...
    async def checkout_record(self, order_id):
        if SessionFactory is None: raise RuntimeError("DATABASE_URL is not configured")
        async with SessionFactory() as session:
            return await session.scalar(select(CheckoutRecord).where(CheckoutRecord.tenant_id==require_tenant(),CheckoutRecord.order_id==order_id))
    ...
    async def set_status(self, order_id, status):
        ...
        async with SessionFactory() as session:
            ...
            record=await session.scalar(select(CheckoutRecord).where(CheckoutRecord.tenant_id==tenant_id,CheckoutRecord.order_id==order.id))
            redemption=await session.scalar(select(DiscountRedemption).where(DiscountRedemption.tenant_id==tenant_id,DiscountRedemption.order_id==order.id))
            old=order.status
