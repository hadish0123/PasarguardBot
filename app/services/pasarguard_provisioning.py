...
class PasarguardProvisioningService:
    async def provision_paid_order(self, order_id: int) -> ServiceSubscription:
        ...
        async with SessionFactory() as session:
            ...
            subscription = await session.scalar(select(ServiceSubscription).where(ServiceSubscription.order_id == order.id, ServiceSubscription.tenant_id == tenant_id))
            if subscription is None:
                subscription = ServiceSubscription(
                    tenant_id=tenant_id, order_id=order.id, telegram_user_id=order.telegram_user_id,
                    plan_id=order.plan_id, plan_name=order.plan_name, volume_gb=order.volume_gb,
                    days=order.days, status="pending_provisioning", provider="pasarguard",
                )
                session.add(subscription)
                await session.flush()
            if subscription.status == "active":
                return subscription
            plan = await session.scalar(select(Plan).where(Plan.id == order.plan_id, Plan.tenant_id == tenant_id))
            tenant = await session.get(TenantRecord, tenant_id)
            if plan is None or tenant is None:
                raise LookupError("plan or tenant not found")
            ...
            box = get_secret_box()
            client = PasarguardClient(tenant.panel_url, box.decrypt(tenant.panel_api_key_encrypted))
            username = f"tg_{order.telegram_user_id}_{order.id}"
            note = f"Representative tenant={tenant_id}; order={order.id}; telegram_user={order.telegram_user_id}"
            try:
                provisioned = await client.create_user_from_template(plan.provider_template_id, username, note)
            except Exception:
                subscription.status = "pending_provisioning"
                await session.commit()
                raise
