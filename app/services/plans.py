...
class PlanService:
    ...
    async def create(self, name: str, volume_gb: float, days: int, price: float) -> Plan:
        if not name.strip() or volume_gb <= 0 or days <= 0 or price < 0:
            raise ValueError("invalid plan values")
        if SessionFactory is None:
            raise RuntimeError("DATABASE_URL is not configured")
        async with SessionFactory() as session:
            plan = Plan(tenant_id=require_tenant(), name=name.strip()[:120], volume_gb=volume_gb, days=days, price=price, enabled=True)
            session.add(plan)
