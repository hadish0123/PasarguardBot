...
from app.telegram.keyboards.buy import build_buy_username_prompt_rows, buy_back_button, buy_cancel_button
...
async def select_duration_group_for_buy_callback(event: events.CallbackQuery.Event):
    ...
    await event.edit(panel_volume_text.replace("{panel_name}", selected_panel.name), buttons=await generate_volume_buttons(panel_code, duration_group=[duration_value]))
...
async def custom_buy_callback(event: events.CallbackQuery.Event):
    ...
    if not panel or settings is None or not is_custom_buy_ready(settings):
        ...
        raise events.StopPropagation
    await clear_custom_buy_session(event.sender_id)
    await set_data_many(event.sender_id, {"panel": panel.code, "selected_plan_id": CUSTOM_PLAN_ID})
    await set_step(event.sender_id, "custom_buy_enter_gb")
    gb_prompt = await _format_bot_text(
        key="custom_buy_enter_gb_message",
        default=("**🧩 خرید دلخواه — {panel_name}**\n\n💰 هر گیگ: `{price_per_gb}` تومان\n"
                 "⏰ هر روز: `{price_per_day}` تومان\n\n📥 حجم مورد نظر را به گیگ وارد کنید (بین `{min_gb}` تا `{max_gb}`):"),
        lang=await _user_lang(event.sender_id), panel_name=panel.name,
        price_per_gb=f"{settings['price_per_gb']:,}", price_per_day=f"{settings['price_per_day']:,}",
        min_gb=format_gb_value(settings["min_gb"]), max_gb=format_gb_value(settings["max_gb"]),
    )
    await event.edit(gb_prompt, buttons=[[await buy_back_button(f"BuyVPN_{panel.code}")], [await buy_cancel_button("DataCancel")]])
...
async def apply_buy_discount_callback(event: events.CallbackQuery.Event):
    if await get_step(event.sender_id) != "crconf":
        await notify_session_expired(event)
        return
    mag_id = await event.edit("**🎉 کد تخفیف جادویی خود را وارد کنید!**\n💰 برای اعمال تخفیف ویژه، کد خود را همین حالا ارسال کنید! 🚀", parse_mode="md", link_preview=False, buttons=[[await buy_cancel_button("DataCancel")]])
...
async def confirm_discounted_buy_callback(event: events.CallbackQuery.Event):
    ...
    if plan is None or new_price is None or code_takhfif is None:
        await event.edit("خطا: اطلاعات مورد نیاز پیدا نشد.", buttons=await bhome_buttons(event.sender_id, await _user_lang(event.sender_id)))
        raise events.StopPropagation
    try:
    ...
    if not status:
        await event.edit("کد تخفیف معتبر نیست.", buttons=await bhome_buttons(event.sender_id, await _user_lang(event.sender_id)))
        raise events.StopPropagation
...
async def confirm_buy_callback(event: events.CallbackQuery.Event):
    ...
    if sticky:
        await _complete_vpn_purchase(event, amount=discounted_price(plan.price, sticky.discount_percentage), discount_code=sticky.code)
    else:
...
def register(client):
    ...
    client.add_event_handler(select_duration_group_for_buy_callback, events.CallbackQuery(pattern=rb"^SelectDurationGroupForBuy:"))
