...
async def render_user(telegram_user_id: int):
    ...
    text = (
        ...
        f"⚫ منقضی: **{len(expired)}**\n"
        f"📋 مجموع: **{len(services)}**\n\n"
        "سرویس موردنظر را انتخاب کنید:"
    )
    buttons = [
        [Button.inline(f"#{s.id} • {s.plan_name} • {_status(s)}", PREFIX + f"view:{s.id}".encode())]
        for s in services[:10]
    ]
    buttons += [
        [Button.inline("🔄 بروزرسانی", PREFIX + b"list")],
        [Button.inline("🛍 خرید سرویس", b"user:buy")],
        [Button.inline("🔙 فروشگاه", b"user:" + USER_HOME.encode())],
    ]
    return text, buttons
...
async def render_callback(event):
    ...
    if action.startswith("view:"):
        ...
        buttons = []
        if service.subscription_url and status in {"🟢 فعال", "⚫ منقضی‌شده"}:
            buttons.append([Button.url("🔗 لینک اشتراک", service.subscription_url)])
        buttons += [
            [Button.inline("🔄 بروزرسانی", PREFIX + f"view:{sid}".encode())],
