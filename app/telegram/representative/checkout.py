...
async def callback(event):
 async with tenant_dispatch(get_tenant()):
  if not await allowed(event): return await event.answer("دسترسی ندارید.",alert=True)
  a=event.data[len(PREFIX):].decode(errors="ignore"); s=STATE.setdefault(key(get_tenant(),event.sender_id),{})
  if a=="pay":
   ...
   STATE.pop(key(get_tenant(),event.sender_id),None); currency=sales.get("currency","تومان"); r=await ORDER_SERVICE.checkout_record(o.id)
   if o.amount<=0:
   ...
   STATE[key(get_tenant(),event.sender_id)]={"payment_order_id":o.id}
