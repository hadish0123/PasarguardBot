...
async def render(event, plan_id):
    ...
    if not sales.get("sales_enabled", True):
        return await event.answer("فروش در حال حاضر غیرفعال است.", alert=True)
    plan = next((p for p in await PlanService().list() if p.id == plan_id and p.enabled), None)
    if not plan:
        return await event.answer("این پلن دیگر فعال نیست.", alert=True)
    ...
    text = (
        f"🧾 **تأیید خرید**\n\n📦 **{plan.name}**\n"
        f"💾 {plan.volume_gb:g} GB · {plan.days} روز\n"
...
async def callback(event, tenant_id):
    async with tenant_dispatch(tenant_id):
        ...
        if action == "pay":
            ...
            if not plan_id:
                return await event.answer("ابتدا پلن را انتخاب کنید.", alert=True)
            sales = await SALES_SERVICE.snapshot()
            if not sales.get("sales_enabled", True):
                return await event.answer("فروش موقتاً غیرفعال است.", alert=True)
            try:
                order = await ORDER_SERVICE.checkout(event.sender_id, int(plan_id), s.get("code"))
            except (LookupError, ValueError) as exc:
                return await event.answer(str(exc), alert=True)
            ...
            if order.amount <= 0:
                ...
                return await event.edit(
                    f"✅ **سفارش #{order.id} پرداخت شد**\n\n📦 {order.plan_name}\n💳 0 {currency}",
                    buttons=[[Button.inline("📦 سرویس‌های من", b"user:services")], [Button.inline("🏪 فروشگاه", b"user:home")]],
                )
...
async def incoming(event, tenant_id):
    async with tenant_dispatch(tenant_id):
        if not await allowed(event):
            return
        state = STATE.get(key(tenant_id, event.sender_id))
        ...
        if state.get("payment_order_id"):
            try:
                order = await ORDER_SERVICE.submit_payment(int(state["payment_order_id"]), text)
            except (LookupError, ValueError) as exc:
                return await event.respond(f"❌ {exc}")
            STATE.pop(key(tenant_id, event.sender_id), None)
            sales = await SALES_SERVICE.snapshot()
            ...
            if not sales.get("allow_pending_orders", True):
                await ORDER_SERVICE.set_status(order.id, "cancelled")
                return await event.respond("❌ ثبت سفارش معلق غیرفعال است؛ سفارش لغو شد.")
            if not sales.get("require_payment_confirmation", True):
                try:
                    await ORDER_SERVICE.set_status(order.id, "paid")
                    status = "پرداخت‌شده و در حال تحویل"
                except Exception:
                    status = "پرداخت ثبت شد؛ تحویل نیاز به بررسی مدیریت دارد"
            else:
                status = "در انتظار تأیید مدیریت"
            return await event.respond(
                f"✅ پرداخت سفارش **#{order.id}** ثبت شد.\n\n"
                f"💳 **{order.amount:,.2f} {currency}**\n🕐 وضعیت: **{status}**",
                buttons=[[Button.inline("📦 سرویس‌های من", b"user:services")], [Button.inline("🏪 فروشگاه", b"user:home")]],
            )
...
def register(client, tenant_id):
    async def cb(event):
        await callback(event, tenant_id)
    async def msg(event):
