...
PREFIX=b"rep:sales:"; _INPUTS:dict[tuple[str,int],str]={}
 ...
 async def message(event):
  async with tenant_dispatch(tenant_id):
   ...
   try:
    ...
    _INPUTS.pop(key,None); text,buttons=await render(); await event.respond("✅ ذخیره شد."); await event.respond(text,buttons=buttons)
   except ValueError as exc:await event.respond(f"❌ {exc}\n\nمقدار دیگری ارسال کنید یا /cancel بزنید.")
 client.add_event_handler(callback,events.CallbackQuery(func=lambda e: bool(e.data and e.data.startswith(PREFIX)))); client.add_event_handler(callback,events.CallbackQuery(data=b"rep:"+REP_SALES.encode())); client.add_event_handler(message,events.NewMessage(incoming=True))
async def _authorized(event)->bool:return bool(event.is_private and get_tenant() and await RepresentativeDashboardService().is_owner(event.sender_id))
async def render():
 data=await SERVICE.snapshot(); on=lambda v:"🟢 روشن" if v else "🔴 خاموش"; text=("⚙️ **تنظیمات فروش**\n\n" f"🛍 فروش: {on(data['sales_enabled'])}\n" f"💳 تأیید پرداخت: {on(data['require_payment_confirmation'])}\n" f"📦 سفارش معلق: {on(data['allow_pending_orders'])}\n" f"💰 واحد قیمت: **{data['currency']}**\n" f"🆘 پشتیبانی: **@{data['support_username']}**" if data['support_username'] else "⚙️ **تنظیمات فروش**\n\n" f"🛍 فروش: {on(data['sales_enabled'])}\n" f"💳 تأیید پرداخت: {on(data['require_payment_confirmation'])}\n" f"📦 سفارش معلق: {on(data['allow_pending_orders'])}\n" f"💰 واحد قیمت: **{data['currency']}**\n🆘 پشتیبانی: **تنظیم نشده**"); buttons=[[Button.inline(f"🛍 فروش {on(data['sales_enabled'])}",PREFIX+b"sales")],[Button.inline(f"💳 تأیید پرداخت {on(data['require_payment_confirmation'])}",PREFIX+b"payment")],[Button.inline(f"📦 سفارش معلق {on(data['allow_pending_orders'])}",PREFIX+b"pending")],[Button.inline("💰 تغییر واحد قیمت",PREFIX+b"currency")],[Button.inline("🆘 تغییر پشتیبانی",PREFIX+b"support")],[Button.inline("📊 داشبورد",b"rep:"+REP_HOME.encode())]; return text,buttons
