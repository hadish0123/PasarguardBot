...
async def render_callback(event):
 ...
 if action in ("","show"):
  t,b=await render_wallet(event.sender_id);await event.edit(t,buttons=b);return await event.answer()
 if action=="history":
  ...
  await event.edit(text,buttons=[[Button.inline("🔙 کیف پول",PREFIX+b"show")]]);return await event.answer()
