...
async def register_handler(event,tenant_id):
 async with tenant_dispatch(tenant_id):
  ...
  if a.startswith("order:"):
   from app.telegram.representative.checkout import render
   return await render(event,int(a.split(":",1)[1]))
  if a=="services":
  ...
  if a=="trial":
   from app.telegram.representative.trial import render_callback
   return await render_callback(event)
  if a=="referral":
