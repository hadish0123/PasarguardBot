...
async def render(query: str | None = None):
    users = await SERVICE.list(query=query)
    title = "👥 **مدیریت کاربران**"
    if query:
        title += f"\n🔎 جستجو: `{query}`"
    if not users:
        text = title + "\n\nکاربری با این مشخصات پیدا نشد."
        return text, [[Button.inline("🔎 جستجوی کاربر", PREFIX + b"search")], [Button.inline("📊 داشبورد", BACK)]]
    buttons = []
    for user in users:
        status = "🚫" if user.blocked else "🟢"
        buttons.append([Button.inline(f"{status} {_name(user)[:28]}", PREFIX + f"view:{user.id}".encode())])
...
async def _show_list(event, query: str | None = None):
    text, buttons = await render(query)
...
async def _show_detail(event, user_id: int):
    ...
    if user is None:
        ...
        return
    block_label = "✅ رفع مسدودی" if user.blocked else "🚫 مسدود کردن"
    buttons = [
        [Button.inline(block_label, PREFIX + f"toggle:{user.id}".encode())],
        [Button.inline("💰 افزایش موجودی", PREFIX + f"balance:{user.id}:add".encode()), Button.inline("💸 کاهش موجودی", PREFIX + f"balance:{user.id}:sub".encode())],
        [Button.inline("👥 بازگشت به کاربران", PREFIX + b"list")],
...
async def callback_handler(event):
    ...
    if raw == "list":
        ...
        return
    if raw == "search":
        _STATES[(get_tenant(), event.sender_id)] = {"step": "search"}
        await event.edit("🔎 **جستجوی کاربر**\n\nTelegram ID، یوزرنیم یا نام را ارسال کنید.\n\nبرای لغو: `لغو`", buttons=[[Button.inline("❌ لغو", PREFIX + b"list")]])
        await event.answer()
        return
    if raw.startswith("view:"):
        await _show_detail(event, int(raw.split(":", 1)[1]))
        await event.answer()
        return
    if raw.startswith("toggle:"):
        user = await SERVICE.toggle_block(int(raw.split(":", 1)[1]))
        await _show_detail(event, user.id)
        await event.answer("وضعیت کاربر به‌روزرسانی شد.")
        return
    if raw.startswith("balance:"):
        _, user_id, direction = raw.split(":")
        _STATES[(get_tenant(), event.sender_id)] = {"step": "balance", "user_id": int(user_id), "direction": direction}
        label = "افزایش" if direction == "add" else "کاهش"
        await event.edit(f"💰 **{label} موجودی**\n\nمبلغ را به‌صورت عدد ارسال کنید.\nمثال: `100000`\n\nبرای لغو: `لغو`", buttons=[[Button.inline("❌ لغو", PREFIX + f"view:{user_id}".encode())]])
        await event.answer()
        return
...
async def message_handler(event):
    ...
    if text.lower() in {"لغو", "/cancel", "❌"}:
        ...
        text_out, buttons = await render()
        await event.respond(text_out, buttons=buttons)
        return
    if state["step"] == "search":
        _STATES.pop(key, None)
        text_out, buttons = await render(text)
        await event.respond(text_out, buttons=buttons)
        return
    if state["step"] == "balance":
        try:
            ...
            await event.respond("❌ مبلغ نامعتبر است. فقط عدد وارد کنید.")
            return
        if amount <= 0:
        ...
        try:
            ...
            return
        _STATES.pop(key, None)
        await event.respond(f"✅ موجودی با موفقیت تغییر کرد.\n\nموجودی جدید: **{user.balance:,.2f}**", buttons=[[Button.inline("👤 مشاهده کاربر", PREFIX + f"view:{user.id}".encode())], [Button.inline("👥 کاربران", PREFIX + b"list")]])
