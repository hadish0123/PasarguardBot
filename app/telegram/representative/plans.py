...
def register(client) -> None:
    client.add_event_handler(plan_text, events.NewMessage(incoming=True))
    client.add_event_handler(plan_callback, events.CallbackQuery(data=PREFIX))
...
async def plan_callback(event) -> None:
    ...
    if action.startswith("toggle:"):
        await SERVICE.toggle(int(action.split(":", 1)[1]))
        await show(event)
        return
    if action.startswith("delete_prompt:"):
        ...
        if not plan:
            ...
            return
        await event.edit(
            f"⚠️ حذف پلن **{plan.name}** قطعی است. ادامه می‌دهید؟",
            buttons=[
    ...
    try:
        if step == "name":
            if len(text) < 2 or len(text) > 120:
                raise ValueError("نام باید بین 2 تا 120 کاراکتر باشد.")
            state.update(name=text, step="volume")
            await event.respond("💾 حجم پلن را به GB وارد کنید. مثال: `0.5`")
            return
        if step == "volume":
            value = float(text)
            if value <= 0:
                raise ValueError("حجم باید بیشتر از صفر باشد.")
            state.update(volume=value, step="days")
            await event.respond("⏱ مدت پلن را به روز وارد کنید. مثال: `30`")
            return
        if step == "days":
            value = int(text)
            if value <= 0:
                raise ValueError("مدت باید بیشتر از صفر باشد.")
            state.update(days=value, step="price")
            await event.respond("💰 قیمت پلن را وارد کنید. عدد صفر هم مجاز است.")
            return
        if step == "price":
            value = float(text)
            if value < 0:
            ...
            _STATES.pop(key, None)
            await event.respond(f"✅ پلن **{plan.name}** با موفقیت ساخته شد.")
            return
