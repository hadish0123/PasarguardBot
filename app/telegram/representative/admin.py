...
from app.core.ids import REP_DISCOUNTS, REP_HOME, REP_LINKS, REP_LOGS, REP_ORDERS, REP_PANEL, REP_PLANS, REP_SALES, REP_SETTINGS, REP_SUPPORT, REP_TEXTS, REP_USERS
...
async def admin_callback(event):
 ...
 data=event.data; child=(b"rep:plans:",b"rep:users:",b"rep:orders:",b"rep:discounts:",b"rep:sales:",b"rep:texts:",b"rep:logs:",b"rep:links:",b"rep:settings:",b"rep:panel:",b"rep:support:")
 if data.startswith(child): return
 action=data[len(PREFIX):].decode(errors="ignore")
 if action==REP_HOME: t,b=await _page("admin","dashboard"); await event.edit(t,buttons=b); return await event.answer()
 pages={REP_PLANS:"plans",REP_USERS:"users",REP_ORDERS:"orders",REP_DISCOUNTS:"discounts",REP_SALES:"sales",REP_TEXTS:"texts",REP_LOGS:"logs",REP_LINKS:"links",REP_SETTINGS:"settings",REP_PANEL:"panel"}
 if action in pages: t,b=await _page(pages[action],"render"); await event.edit(t,buttons=b); return await event.answer()
