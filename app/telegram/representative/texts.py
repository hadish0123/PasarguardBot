...
def register(client,tenant_id=None):
 ...
 client.add_event_handler(callback,events.CallbackQuery(func=lambda e: bool(e.data and e.data.startswith(PREFIX)))); client.add_event_handler(callback,events.CallbackQuery(data=b"rep:"+REP_TEXTS.encode())); client.add_event_handler(message,events.NewMessage(incoming=True))
...
async def render():
 ...
 rows.append([Button.inline("📊 داشبورد",b"rep:"+REP_HOME.encode())]); return "📝 **متن‌ها و دکمه‌ها**\n\nیک مورد را برای مشاهده و ویرایش انتخاب کنید.",rows
async def detail(key):
 vals=await SERVICE.all(); return f"📝 **{LABELS[key]}**\n\n`{key}`\n\n{vals[key]}",[[Button.inline("✏️ ویرایش",PREFIX+b"edit:"+key.encode()),Button.inline("♻️ بازنشانی",PREFIX+b"reset:"+key.encode())],[Button.inline("🔙 فهرست",PREFIX+b"list")]]
