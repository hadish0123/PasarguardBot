...
def register(client,tenant_id=None):
 ...
 async def msg(event):
  async with tenant_dispatch(tenant_id):
   ...
   try:await SERVICE.set(field,value);INPUTS.pop(key,None);t,b=await render();await event.respond("✅ تنظیمات ذخیره شد.",buttons=b)
