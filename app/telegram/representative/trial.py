...
async def render_callback(event):
 ...
 if action in ("","claim"):
  try:plan,subscription=await SERVICE.claim(event.sender_id)
  except (ValueError,LookupError) as exc:return await event.edit(f"🎁 **سرویس آزمایشی**\n\n❌ {exc}",buttons=[[Button.inline("🔙 فروشگاه",HOME_CALLBACK)]])
  except Exception as exc:return await event.edit(f"🎁 **سرویس آزمایشی**\n\n❌ ساخت سرویس در پاسارگارد انجام نشد.\n\n{str(exc)[:300]}",buttons=[[Button.inline("🔄 تلاش دوباره",PREFIX+b"claim")],[Button.inline("🔙 فروشگاه",HOME_CALLBACK)]])
  rows=[[Button.inline("📦 سرویس‌های من",b"user:services")],[Button.inline("🏪 فروشگاه",HOME_CALLBACK)]]
