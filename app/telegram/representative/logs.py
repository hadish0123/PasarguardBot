...
def register(client,tenant_id=None):
 async def callback(event):
  async with tenant_dispatch(tenant_id):
   ...
   if event.data==b"rep:"+REP_LOGS.encode() or event.data==PREFIX+b"list": await render_into(event); return
   await event.answer("گزینه نامعتبر است.",alert=True)
 client.add_event_handler(callback,events.CallbackQuery(data=PREFIX)); client.add_event_handler(callback,events.CallbackQuery(data=b"rep:"+REP_LOGS.encode()))
...
async def render_into(event):
 t,b=await render(); await event.edit(t,buttons=b); await event.answer()
