...
async def render():
    ...
    if not items: text="🎟 **تخفیف‌ها**\n\nهنوز کد تخفیفی ساخته نشده است."
    else: text="🎟 **تخفیف‌ها**\n\n"+"\n".join(f"• {label(x)} | استفاده: {x.used_count}/{x.max_uses if x.max_uses else '∞'}" for x in items)
    rows=[[Button.inline("➕ ساخت کد تخفیف",PREFIX+b"add")]]
    rows += [[Button.inline(label(x),PREFIX+f"view:{x.id}".encode()),Button.inline("🔄",PREFIX+f"toggle:{x.id}".encode())] for x in items]
    rows.append([Button.inline("🔙 داشبورد",BACK)])
    return text,rows

async def callback(e):
    if not await owner(e): return await e.answer("دسترسی ندارید.",alert=True)
    a=e.data[len(PREFIX):].decode()
    if a in ('','list'):
        t,b=await render(); await e.edit(t,buttons=b); return await e.answer()
    key=(get_tenant(),e.sender_id)
    if a=='add':
        STATES[key]={'step':'code'}; await e.edit("➕ **ساخت کد تخفیف**\n\nکد را ارسال کنید.\nمثال: `WELCOME10`",buttons=[[Button.inline("❌ لغو",PREFIX+b"cancel")]]); return await e.answer()
    ...
    if a.startswith('view:'):
        ...
        exp=d.expires_at.strftime('%Y-%m-%d %H:%M') if d.expires_at else 'بدون انقضا'
        t=f"🎟 **{d.code}**\n\n📉 تخفیف: **{d.percent:g}%**\n🔢 استفاده: **{d.used_count}/{d.max_uses if d.max_uses else '∞'}**\n⏳ انقضا: **{exp}**\n📌 وضعیت: **{'فعال' if d.enabled else 'غیرفعال'}**"
        b=[[Button.inline("🔄 تغییر وضعیت",PREFIX+f"toggle:{d.id}".encode())],[Button.inline("🗑 حذف",PREFIX+f"delete_confirm:{d.id}".encode())],[Button.inline("🔙 لیست",PREFIX+b"list")]]
        await e.edit(t,buttons=b); return await e.answer()
    if a.startswith('toggle:'):
        await SERVICE.toggle(int(a.split(':')[1])); t,b=await render(); await e.edit(t,buttons=b); return await e.answer("وضعیت تغییر کرد.")
    ...
    if a.startswith('delete:'):
        await SERVICE.delete(int(a.split(':')[1])); t,b=await render(); await e.edit(t,buttons=b); return await e.answer("حذف شد.")
...
async def message(e):
    ...
    try:
        ...
        if s['step']=='expires':
            exp=None if text=='0' else datetime.strptime(text,'%Y-%m-%d %H:%M')
            d=await SERVICE.create(s['code'],s['percent'],s['max_uses'],exp); STATES.pop(key,None)
            await e.respond(f'✅ کد **{d.code}** ساخته شد.',buttons=[[Button.inline('🎟 تخفیف‌ها',PREFIX+b'list')]]); return
