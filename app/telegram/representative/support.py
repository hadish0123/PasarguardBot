...
async def render(event):
 settings=await SETTINGS.snapshot();username=settings.get("support_username") or "";contact=f"@{username.lstrip('@')}" if username else "از طریق نماینده پشتیبانی پیام ارسال کنید."
