...
def register(client, tenant_id=None):
    ...
    client.add_event_handler(callback, events.CallbackQuery(data=b"rep:panel"))
...
async def render(status=None):
    ...
    text = (
        ...
        f"ℹ️ {status.message}\n\n"
        "🔐 کلید API به‌صورت امن ذخیره شده و در این صفحه نمایش داده نمی‌شود."
    )
    buttons = [
...
async def handle(event):
    ...
    if action == "check":
        ...
        return await event.answer(
            "اتصال برقرار است." if status.state == "connected" else "نتیجه تست اتصال نمایش داده شد."
        )
    if action in ("list", ""):
