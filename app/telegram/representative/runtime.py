...
class RepresentativeRuntime:
 def __init__(self,tenant_id:str,bot_token:str):
  self.tenant_id=tenant_id; self.bot_token=bot_token.strip(); self.client=TelegramClient(f"tenant-{tenant_id}"); self.is_running=False; self._polling_task:asyncio.Task|None=None; self.dashboard=RepresentativeDashboardService()
 def register(self):
 ...
 async def _force_join_message(self,event):
  async with tenant_dispatch(self.tenant_id):
   ...
   await event.respond("🔒 **عضویت اجباری**\n\nبرای استفاده از ربات ابتدا در کانال اعلام‌شده عضو شوید و سپس «بررسی عضویت» را بزنید.",buttons=buttons)
 ...
 async def _force_join_callback(self,event):
  async with tenant_dispatch(self.tenant_id):
   ...
   if data!=b"forcejoin:check":
    ...
    return
   if await self._force_join_required(event): return await event.answer("هنوز عضویت شما تأیید نشده است.",alert=True)
   await event.answer("✅ عضویت تأیید شد.",alert=True)
   try: await event.respond("اکنون می‌توانید از ربات استفاده کنید.")
   except Exception:pass
   try:event.stop_propagation()
   except Exception:pass
 async def _start(self,event):
  async with tenant_dispatch(self.tenant_id):
   if await self._force_join_required(event): return
   await asyncio.gather(self.client.clear_bot_messages(event.chat_id),event.delete(),return_exceptions=True)
   ...
   if ref_arg and ref_arg.strip().startswith("ref_"):
    try:await REFERRAL_SERVICE.attach(int(ref_arg.strip()[4:]),event.sender_id)
    except (ValueError,LookupError):pass
   values=await TEXT_SERVICE.all(); from app.telegram.representative.navigation import customer_menu
 ...
 async def start(self):
  ...
  try:
   ...
   logger.info("[representative-runtime] initialized tenant=%s bot_id=%s bot_username=%s token_fp=%s",self.tenant_id,getattr(getattr(self.client,"_me",None),"id","unknown"),getattr(getattr(self.client,"_me",None),"username","") or "",_token_fingerprint(self.bot_token))
   self._polling_task=asyncio.create_task(self.client.run_until_disconnected(),name=f"representative-poll-{self.tenant_id}"); self._polling_task.add_done_callback(self._polling_done); self.is_running=True
  except Exception:
   await self.client.disconnect(); self._polling_task=None; self.is_running=False; raise
 def _polling_done(self,task:asyncio.Task)->None:
  if self._polling_task is not task:return
  if task.cancelled():return
  exc=task.exception()
  if exc is not None:logger.error("[representative-runtime] polling task exited tenant=%s token_fp=%s error=%s",self.tenant_id,_token_fingerprint(self.bot_token),exc,exc_info=exc)
  else:logger.warning("[representative-runtime] polling task exited unexpectedly tenant=%s",self.tenant_id)
  self.is_running=False
 async def stop(self):
  task=self._polling_task; self._polling_task=None; self.is_running=False
