...
def _panel_command_filter(event: Message) -> bool:
    if not is_runtime_admin(event.sender_id) or not event.is_private:
        return False
...
async def admin_menu_entry_handler(event: Message):
    ...
    if msg == "🔗 لینک های آماده":
        ...
        await event.respond(format_admin_links_message(bot_username), buttons=[[Button.inline("🔙 بازگشت به پنل", data="back_to_admin_panel")]], parse_mode="md")
        return
    user = await event.get_sender()
...
def register(client):
    client.add_event_handler(message_handler_admin_panel, events.NewMessage(incoming=True, func=_panel_command_filter))
    client.add_event_handler(admin_menu_entry_handler, events.NewMessage(incoming=True, func=_admin_menu_entry_filter))
