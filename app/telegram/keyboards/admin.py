...
async def create_inline_manageuser(UserID):
    ...
    buttons = [[Button.inline("📂 لیست سرویس‌ها", f"MToUser_listSv:{UserID}")], [Button.inline("🏢 نمایندگی‌های کاربر", f"MToUser_resellers:{UserID}")], [Button.inline("🔍 جستجوی کانفیگ", f"AdminSearchConfig:{UserID}"), Button.inline("🗑 حذف گروهی کانفیگ", f"BulkDeleteConfigs:{UserID}")], [Button.inline("رفع مسدودی", f"unbansup_{UserID}"), Button.inline("مسدود سازی", f"bansup_{UserID}")], [Button.inline("اطلاعات کاربر", f"UserInfo:{UserID}"), Button.inline("پیام به کاربر", f"sendm_{UserID}")]]
...
REPRESENTATIVE_PANEL_ADMIN_BUTTONS = [
    ...
    [create_button("🎟 کدهای تخفیف"), create_button("👥 کاربران")],
    [create_button("📝 متن‌ها و دکمه‌ها"), create_button("📝 مدیریت لاگ‌ها")],
