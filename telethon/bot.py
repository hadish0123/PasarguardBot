...
def _button_markup(buttons):
    ...
    for row in buttons:
        ...
        if out:
            rows.append(out)
    # Preserve the legacy distinction between inline and reply keyboards.
...
class TelegramClient:
    ...
    async def get_messages(self, entity, ids=None, limit=1, **kwargs):
        # Bot API does not expose arbitrary history. Return None/empty rather than
        # pretending that MTProto history is available.
    ...
    async def forward_messages(self, entity, messages, from_peer=None, **kwargs):
        ...
        result = await self._api("forwardMessages", chat_id=_chat_id(entity), from_chat_id=_chat_id(from_peer), message_ids=mids)
        return result
