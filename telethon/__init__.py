...
class TelegramClient:
    ...
    async def send_document(self, entity, document, *, filename: str = "config.txt", caption: str = "", **kwargs):
        if not self._session or not self._base_url:
            raise RuntimeError("Telegram client is not started")
        ...
        async with self._session.post(f"{self._base_url}/sendDocument", data=form) as response:
            data = await response.json(content_type=None)
            if not data.get("ok"):
                raise RuntimeError(f"Telegram Bot API sendDocument failed: {data.get('description', data)}")
