...
ADMIN_ID: list[int] = config("ADMIN_ID", cast=lambda v: [int(i.strip()) for i in v.split(",") if i.strip()])
...
LOG_FORMAT = config("LOG_FORMAT", default="%(asctime)s | %(levelname)-8s | %(message)s")
